...
const parse = {
    ...
    fieldArgument(item, model, options){
        if(model.fieldsMap[item]){
            return [{table: model.alias, field: model.fieldsMap[item].name }];
        }

        const refs = parse.followReferences(item, model, options);
        return refs && [{ table: refs.lastModel.alias, field: refs.field }, refs]
    },

    valueArgument(item, model, options){
        if (item.field) {
            return parse.showArgument(item.field);
        } else if(item.func || Object.keys(item).length === 1){
            return parse.showArgument(item);
        } else {
            return parse.showArgument({asIs: item});
        }
    },
    ...
    conditionsList(conditionsList, model, tableList, options) {
        const parsed = [];
        Object.entries(conditionsList).forEach(([key, option]) => {
            if(option === undefined){
                return;
            }

            const field = model.fieldsMap[key];
            let condition = null;
            if (field) {
                condition = parse.condition(model.alias, field.name, option)
            } else {
                const refs = parse.followReferences(key, model, options);
                if (refs) {
                    if (tableList) {
                        refs.path.forEach(component => tableList.addTable(parse.join(component)));
                    }
                    condition = parse.condition(refs.lastModel.alias, refs.field, option);
                }
            }

            if (condition){
                parsed.push(...parse.asArray(condition).filter(c => !!c));
            }
        });
        return parsed;
    },
