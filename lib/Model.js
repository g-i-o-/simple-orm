...
class Model {
    ...
    /** deletes already-existing entities of this model.
     * @params {Object} options - options object
     * @params {Array} where - set of conditions limiting the entity deletion.
     * @params {Transaction} transaction - an ongoing transaction, if any
     */
    async delete(options) {
        ...
        const db = options.transaction || await this.getConnection();
        if (!db) {
            throw new Error(`Cannot delete ${this.name} objects, invalid connection.`);
        }

        const model = this; // no prefix aliases thank you

        const tableList = new TableListBuilder([{ table: model.name, as: model.alias }]);
        let delTableList = tableList;
        const updatedFields = [];
        const where = [];

        if (options.delete) {
            delTableList = new TableListBuilder([{ table: model.name, as: model.alias }]);
            parse.asArray(options.delete).forEach((item) => {
                const refs = parse.followReferences(item.field || item, model, options);
                if (refs) {
                    refs.path.forEach(component => {
                        tableList.addTable(parse.join(component));
                        delTableList.addTable(parse.join(component));
                    });
                }
            });
        }
        ...
        ['orderBy', 'limit', 'transaction'].forEach((k) => { delete options[k]; });
