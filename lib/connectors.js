...
module.exports.getConnectorFor = (schema) => {
    const databaseType = schema.database;
    ...
    if (!module) {
        return new Error(`Unknown database type ${databaseType}.`);
    }

    const ConnectorClass = require(module);
    return new ConnectorClass(schema);
};
