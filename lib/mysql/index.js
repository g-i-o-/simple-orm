...
class mysqlConnector {
    ...
    /** Returns a promise, resolving to the connection wrapper, primed andready for connecting to a mysql database.
     * @returns {Promise} resolving to the connection wrapper.
     */
    ...
    /** Disconnects the connector from the database.
    ...
    disconnect() {
        ...
        return new Promise((resolve, reject) => connection.connection.end((err) => {
            if (err) {
                reject(err);
            } else {
                resolve();
